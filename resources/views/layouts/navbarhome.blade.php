    ...
    <div class="container mx-auto flex justify-between items-center flex-wrap">
        ...
        <div :class="{'hidden': !isOpen, 'block': isOpen}" class="w-full lg:flex lg:items-center lg:w-auto">
            <ul class="lg:flex space-y-2 lg:space-y-0 lg:space-x-4">
                ...
                @if($item->is_dropdown)
                <li class="relative">
                    <button @click="isOpen{{ $item->id }} = !isOpen{{ $item->id }}"
                        class="block text-black hover:text-gray-200 text-sm">{{ $item->title }}</button>
                    <ul :class="{'hidden': !isOpen{{ $item->id }}, 'block': isOpen{{ $item->id }}}"
                        class="absolute left-0 w-40 mt-2 bg-white border rounded-lg shadow-lg">
